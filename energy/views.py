...
def whatif(request):
    battery_kwh = float(request.GET.get('battery_kwh', 400))
    pv_scale    = float(request.GET.get('pv_kw', 200)) / 200.0
    ...
    # Baseline run — 400 kWh, 200 kWp
    baseline_solar = [solar_map.get(ts, 0.0) for ts in timestamps]
    baseline_results = run_dispatch(timestamps, baseline_solar, load_kws, prices,
        battery_capacity_kwh=400, battery_max_power_kw=200)
    baseline_saving = sum(r.no_battery_cost_eur - r.grid_cost_eur for r in baseline_results)

    # New run — user parameters
    scaled_solar = [solar_map.get(ts, 0.0) * pv_scale for ts in timestamps]
    results = run_dispatch(timestamps, scaled_solar, load_kws, prices,
        battery_capacity_kwh=battery_kwh,
        battery_max_power_kw=min(battery_kwh / 2, 200),
    )

    total_grid_cost        = sum(r.grid_cost_eur       for r in results)
    total_no_battery_cost  = sum(r.no_battery_cost_eur for r in results)
    total_saving           = total_no_battery_cost - total_grid_cost
    saving_delta           = total_saving - baseline_saving
    ...
    return render(request, 'energy/whatif.html', context)